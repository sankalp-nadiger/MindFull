        ...
        <div className="max-w-6xl px-4 py-4 mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link 
                to="/MainPage" 
                className={`flex items-center gap-2 ${themeClasses.text} hover:text-blue-400 transition-all duration-200 hover:scale-105`}
              >
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm ${themeClasses.textSecondary}`}>AI Assistant Online</span>
              </div>
              
              {messages.length > 1 && (
                <button
                  onClick={clearChat}
                  className={`px-4 py-2 text-sm ${themeClasses.cardBg} hover:bg-red-500/10 rounded-xl transition-all border text-red-400 hover:text-red-500 hover:scale-105`}
                >
                  Clear Chat
                </button>
              )}
              ...
              <button
                ...
                className={`p-3 rounded-xl ${themeClasses.cardBg} transition-all hover:scale-105`}
              >
          ...
          <div className="flex items-center gap-4 mt-4">
            <div className="flex items-center justify-center shadow-lg w-14 h-14 bg-gradient-to-r from-blue-500 to-purple-500 rounded-2xl">
              <Bot className="text-white w-7 h-7" />
            </div>
            <div>
              <h1 className={`text-3xl font-bold ${themeClasses.text} bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent`}>
                MindFull Bot
              </h1>
              <p className={`text-sm ${themeClasses.textSecondary}`}>Your compassionate AI mental health companion</p>
            </div>
          </div>
        </div>
      </div>      {/* Enhanced Chat Container */}
      <div className="max-w-6xl px-4 py-8 mx-auto">
        <div className={`${themeClasses.cardBg} rounded-3xl shadow-2xl overflow-hidden border`}>
          
          {/* Messages Area */}
          <div className="h-[65vh] overflow-y-auto p-6 space-y-6" style={{ scrollbarWidth: 'thin' }}>
            {messages.length === 0 && (
              <div className="py-16 text-center">
                ...
                <h3 className={`text-2xl font-bold ${themeClasses.text} mb-3`}>Welcome to MindFull Bot</h3>
            ...
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} mb-6`}>
                <div className={`flex ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start gap-4 max-w-[85%]`}>
                  {/* Enhanced Avatar */}
                    ...
                    <div className="px-5 py-4">
                      {msg.role === 'assistant' ? (                        <div className={`prose prose-sm max-w-none markdown-content ${darkMode ? 'dark' : 'light'}`}>
                          <ReactMarkdown
                            ...
                            components={{
                              h1: ({node, ...props}) => <h1 className="mb-3 text-xl font-bold text-blue-400" {...props} />,
                              h2: ({node, ...props}) => <h2 className="mb-2 text-lg font-semibold text-purple-400" {...props} />,
                              h3: ({node, ...props}) => <h3 className="mb-2 font-medium text-md" {...props} />,
                              ...
                              blockquote: ({node, ...props}) => <blockquote className="pl-4 my-3 italic border-l-4 border-purple-400" {...props} />,
                              a: ({node, ...props}) => <a className="text-blue-400 underline hover:text-blue-300" target="_blank" rel="noopener noreferrer" {...props} />,
                              code: ({node, inline, ...props}) => 
                                inline ? 
                                  <code className="bg-slate-600/50 px-1 py-0.5 rounded text-sm" {...props} /> :
                                  <code className="block p-3 overflow-x-auto text-sm rounded-lg bg-slate-700" {...props} />,
                              pre: ({node, ...props}) => <pre className="p-3 mb-3 overflow-x-auto text-sm rounded-lg bg-slate-700" {...props} />
                            }}
                          >
                            {msg.content}
                          </ReactMarkdown>
              ...
              <div className="flex justify-start mb-6">
                <div className="flex items-start gap-4 max-w-[85%]">
                  <div className="flex items-center justify-center flex-shrink-0 w-10 h-10 rounded-full shadow-lg bg-gradient-to-r from-blue-500 to-purple-500">
                    <Bot className="w-5 h-5 text-white" />
                  </div>
                  <div className={`${themeClasses.botBubble} px-5 py-4 rounded-2xl shadow-xl`}>
                    <div className="flex items-center gap-3">
                      <span className={`text-sm ${themeClasses.textSecondary}`}>MindFull Bot is thinking</span>
          ...
          {messages.length <= 1 && (
            <div className="px-6 py-6 border-t border-gray-200/10">
              ...
              <div className="grid grid-cols-1 gap-3 mb-6 md:grid-cols-2 lg:grid-cols-3">
                {quickPrompts.map((prompt, index) => (
                  <button
                    ...
                    className={`p-4 text-left ${themeClasses.cardBg} hover:bg-blue-500/10 rounded-xl transition-all border group hover:scale-105 hover:shadow-lg`}
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-2xl transition-transform group-hover:scale-110">{prompt.icon}</span>
                      <div>
                        <div className={`${themeClasses.text} font-medium mb-1`}>{prompt.text}</div>
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                {mindfulnessExercises.map((exercise, index) => (
                  <button
                    ...
                    className={`p-4 text-left ${themeClasses.cardBg} hover:bg-purple-500/10 rounded-xl transition-all border group hover:scale-105 hover:shadow-lg`}
                  >
                    <div className="text-sm">
                      <div className={`font-semibold ${themeClasses.text} mb-2 group-hover:text-purple-400 transition-colors`}>{exercise.title}</div>
            ...
            <div className="flex items-end gap-4">
              <div className="flex-1">
                <textarea
                  ...
                  className={`w-full p-4 ${themeClasses.input} rounded-2xl border resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all placeholder-gray-400 shadow-lg`}
              ...
              <button
                ...
                className={`px-6 py-4 ${themeClasses.button} text-white rounded-2xl shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-xl transform hover:scale-105 flex items-center gap-2`}
              >
