...
const FileUpload = ({ userId, fetchVisionBoards, darkMode, themeClasses }) => {
 ...
 return (
  <>
    <div className={`file-upload rounded-xl shadow-lg mb-8 w-full max-w-4xl mx-auto overflow-hidden ${themeClasses.card}`}>
      {/* Header bar with proper theme colors */}
      ...
      <div className="p-6">
        ...
        <div 
          ...
          className={`border-2 border-dashed rounded-lg p-8 flex flex-col items-center justify-center cursor-pointer transition-all duration-200 ${
            darkMode 
              ? 'border-indigo-400/50 hover:border-indigo-300 bg-indigo-900/20 hover:bg-indigo-900/30 hover:shadow-lg' 
              : 'border-blue-400/50 hover:border-blue-400 bg-blue-50 hover:bg-blue-100 hover:shadow-md'
          }`}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-12 w-12 mb-3 transition-colors ${
            darkMode ? 'text-indigo-300' : 'text-blue-600'
          }`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <p className={`text-sm font-medium ${
            darkMode ? 'text-indigo-200' : 'text-blue-800'
          }`}>Click to select an image</p>
          <p className={`text-xs ${
            darkMode ? 'text-indigo-300/80' : 'text-blue-600/80'
          } mt-1`}>PNG, JPG, GIF up to 10MB</p>
        </div>
      </div>
    </div>
    ...
    {isModalOpen && (
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className={`rounded-xl shadow-2xl p-6 w-full max-w-xl max-h-[90vh] overflow-auto border ${
          darkMode 
            ...
            <div className="mb-4">
              ...
              <div className={`rounded-lg overflow-hidden border ${
                ...
                <img 
                  ...
                  className="w-full h-40 object-contain bg-gray-50 dark:bg-slate-700"
                />
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleUpload}>
            ...
            <div className="mb-4">
              ...
              >
                {categories.map((cat) => (
                  <option key={cat} value={cat} className={darkMode ? 'bg-slate-700' : 'bg-white'}>
                    {cat}
                  </option>
                ))}
              </select>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded-lg border transition-colors ${
                  darkMode 
                    ? 'border-slate-600 text-slate-300 hover:bg-slate-700 focus:ring-slate-500' 
              ...
              <button
                type="submit"
                className={`px-4 py-2 text-sm font-medium rounded-lg text-white transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                  darkMode
                    ? 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 focus:ring-offset-slate-800 disabled:bg-slate-600'
                    : 'bg-blue-600 hover:bg-blue-700 focus:ring-blue-500 focus:ring-offset-white disabled:bg-gray-400'
